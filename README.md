...
Inbound callbacks are sent notifying you of a received message on a Bandwidth number, this app saves an inbound image to the working directory. Outbound callbacks are status updates for messages sent from a Bandwidth number, this app has a dedicated response for each type of status update.
